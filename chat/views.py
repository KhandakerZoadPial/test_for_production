...
def index(request):
    ...
    if request.user.is_authenticated:
        if Profile.objects.filter(user=request.user).count() > 0:
            ...
            profile = Profile(user=request.user, email_code=code)
            profile.save()
...
def message_list(request, sender=None, receiver=None):
    ...
    if request.method == 'GET':
        ...
        data['private_key'] = e2e.keys()['private']
        encrypted_message = e2e.encrypt(message=data['message'], public_key=e2e.keys()['public'])
...
def message_view(request, sender, receiver):
    ...
    if request.method == "GET":
        profile = Profile.objects.get(user=User.objects.get(email=request.user.email))
        msg_set = Message.objects.filter(sender_id=sender, receiver_id=receiver) | Message.objects.filter(sender_id=receiver, receiver_id=sender)
        # print(msg_set.count())
        for msg in msg_set:
            msg.message = e2e.decrypt(msg.message, private_key=msg.private_key)
            print(msg.message)
